...
describe('Receipt', () => {
  ...
  describe('constructor', () => {
    ...
    it('should convert properties to camelCase', () => {
      expect(this.receipt.properties)
        .to.have.property('calledBackAt');

      expect(this.receipt.properties.calledBackAt)
        .to.equal(this.stubResult.called_back_at);
    });
  });

  describe('get id', () => {
    it('should equal initial value', () => {
      expect(this.receipt.id)
        .to.equal(this.stubId);
    });
  });

  describe('get isAcknowledged', () => {
    it('should equal initial value', () => {
      expect(this.receipt.isAcknowledged)
        .to.equal(this.stubResult.acknowledged);
    });
  });
  ...
  describe('get isExpired', () => {
    it('should equal initial value', () => {
      expect(this.receipt.isExpired)
        .to.equal(this.stubResult.expired);
    });
  });
  ...
  describe('get hasCalledBack', () => {
    it('should equal initial value', () => {
      expect(this.receipt.hasCalledBack)
        .to.equal(this.stubResult.called_back);
    });
  });
