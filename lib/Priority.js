...
const DEFAULT_EMERGENCY_PROPERTIES = {
  'priority': PRIORITY_SET['emergency'],
  'retry':    30,
  'expire':   86400,
  'callback': null
}
...
class Priority {
  ...
  constructor(name, options) {
    ...
    this.properties = {
      'priority': PRIORITY_SET[name]
    };

    if (name == 'emergency') {
      Object.assign(this, new EmergencyPriorityTrait(options));
    }
  }
...
/**
 * Emergency priority trait
 */
class EmergencyPriorityTrait {
  /**
   * Constructor for emergency priority trait
   *
   * @param {object} options Options
   */
  constructor(options) {
    this.properties = DEFAULT_EMERGENCY_PROPERTIES;
